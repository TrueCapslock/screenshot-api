  ...
  _deploy_run() {
    ...
    case "$1" in
      ...
      4) docker_submenu ;;
       ...
       7) echo "Creating Sentry release..."; APP_VERSION=$(node -p "require('./package.json').version" 2>/dev/null || echo "0.0.0"); SENTRY_RELEASE="screenshot-api@${APP_VERSION}"; if [ -f .env ]; then _tok=$(grep '^SENTRY_AUTH_TOKEN=' .env | sed 's/^SENTRY_AUTH_TOKEN=//' | head -1 | xargs); [ -n "$_tok" ] && SENTRY_AUTH_TOKEN=$_tok && export SENTRY_AUTH_TOKEN; unset _tok; SENTRY_ORG=$(grep '^SENTRY_ORG=' .env | sed 's/^SENTRY_ORG=//' | head -1 | xargs); SENTRY_PROJECT=$(grep '^SENTRY_PROJECT=' .env | sed 's/^SENTRY_PROJECT=//' | head -1 | xargs); fi; SENTRY_ORG="${SENTRY_ORG:-anomaly}"; SENTRY_PROJECT="${SENTRY_PROJECT:-screenshot-api}"; if [ -z "$SENTRY_AUTH_TOKEN" ]; then echo "  $R⚠ SENTRY_AUTH_TOKEN not set — skipping Sentry release$X"; else
